...
namespace WinFormUtilityLibrary.DataUtilities
{
    public static class ISO3166
    {
        ...
        private static Dictionary<string, string> mapCurrencySymbol;
        private static IEnumerable<KeyValuePair<string, string>> map2LetteredSymbol;

        static ISO3166()
        {
            collection = BuildCollection();

            mapCurrencySymbol = CultureInfo
            ...
            map2LetteredSymbol = CultureInfo
            .GetCultures(CultureTypes.AllCultures)
            .Where(c => !c.IsNeutralCulture)
            .Select(culture => {
                try
            ...
            .Where(ri => ri != null)
            .Select(x => new KeyValuePair<string,string>(x.ISOCurrencySymbol, x.ThreeLetterISORegionName) );
        ...
        public static bool TryGetCurrencySymbol(
                          ...
                          out string symbol)
        {
            return mapCurrencySymbol.TryGetValue(ISOCurrencySymbol, out symbol);
        }

        public static int GetISONumericCurrencyCode(string ISOCurrencySymbol)
        {
            string shortname = map2LetteredSymbol.FirstOrDefault(k => k.Key.ToLower() == ISOCurrencySymbol.ToLower()).Value;

            if (shortname == null) return 0;

            var ctr = collection.FirstOrDefault(c => c.Alpha3.ToLower() == shortname.ToLower());

            return ctr == null ? 0 : ctr.NumericCode;
        }
